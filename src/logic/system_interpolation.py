		...
		for eid, comp_tuple in component_tuples:
			...
			if comp_transform.server_modified:
				...
				if comp_interpolation.mode == const.REPLICA_INTERPOLATION_LINEAR:
					...
					h = comp_transform.server_position - comp_transform.position
					comp_interpolation.cubic_args[0] = comp_transform.position
					comp_interpolation.cubic_args[1] = comp_transform.velocity
					comp_interpolation.cubic_args[2] = (h * 3 - (comp_transform.velocity * 2 + comp_transform.server_velocity) * t) / (t ** 2)
					comp_interpolation.cubic_args[3] = (h * -2 + (comp_transform.velocity + comp_transform.server_velocity) * t) / (t ** 3)
			...
			if comp_interpolation.remain_t <= dt:
				...
				if comp_interpolation.mode == const.REPLICA_INTERPOLATION_LINEAR:
					...
					comp_transform.velocity = (comp_transform.server_position - comp_transform.position) / comp_interpolation.remain_t
